...
public class BeeModelBuilder extends CustomLoaderBuilder<ItemModelBuilder> {
    ...
    public BeeModelBuilder variant(ResourceLocation species, ResourceLocation modelFile) {
        variantMap.put(species, modelFile);
        return this;
    }

    public BeeModelBuilder emissive(int blockLight, int skyLight, int... layers) {
     ...
     */
    public BeeModelBuilder color(int color, int... layers) {
        Preconditions.checkNotNull(layers, "Layers must not be null");
        Preconditions.checkArgument(layers.length > 0, "At least one layer must be specified");
        Preconditions.checkArgument(Arrays.stream(layers).allMatch(i -> i >= 0), "All layers must be >= 0");
        for (int i : layers) {
     ...
     */
    public BeeModelBuilder renderType(String renderType, int... layers) {
     ...
     * @throws IllegalArgumentException if any entry in {@code layers} already has a render type
     */
    public BeeModelBuilder renderType(ResourceLocation renderType, int... layers) {
        Preconditions.checkNotNull(renderType, "Render type must not be null");
        Preconditions.checkNotNull(layers, "Layers must not be null");
        Preconditions.checkArgument(layers.length > 0, "At least one layer must be specified");
        Preconditions.checkArgument(Arrays.stream(layers).allMatch(i -> i >= 0), "All layers must be >= 0");
    ...
    public JsonObject toJson(JsonObject json) {
        ...
        for (Map.Entry<ResourceLocation, ResourceLocation> entry : variantMap.entrySet()) {
            variants.add(entry.getKey().toString(), new JsonPrimitive(entry.getValue().toString()));
        }
